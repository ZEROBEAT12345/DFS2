...
{
	float f = 1.0 - roughness;
    return F0 + (max(float3(f, f, f), F0) - F0) * pow(1.0 - cosTheta, 5.0);
} 
...
float3 Get_IBL_BRDF_Lighting(float3 eye_pos,
	float3 surface_position,
	float3 surface_normal,
	float3 surface_albedo,
	float surface_metallic,
	float surface_roughness,
	float3 surface_irradiance,
	float3 surface_preFilter,
	float2 surface_envBRDF)
{
	...
	// Add IBL light as ambient
	float3 F = Fresnel_Schlick_Roughness(saturate(dot(N, V)), F0, surface_roughness);

	float3 kS = F;
	float3 kD = 1.0 - kS;
	kD = kD * (1.0f - surface_metallic);  	  
  
	float3 diffuse  = surface_irradiance * surface_albedo;
	float3 specular = surface_preFilter * (F * surface_envBRDF.x + surface_envBRDF.y);
