...
cbuffer time_constants : register(b1)
{
	float Time;
	float halfTime;
	float cosTime;
...
v2f_t VertexFunction( vs_input_t input )
{
   ...
   clip_x += 1.0f * (cosTime * 0.5f + 0.5f);
   clip_y += 1.0f * (cosTime * 0.5f + 0.5f);
...
float4 FragmentFunction( v2f_t input ) : SV_Target0
{
   ...
   float4 blend = float4(1.0f * (cosTime * 0.5f + 0.5f), 1.0f * (sinTime * 0.5f + 0.5f), 0.0f, 1.0f) * input.color;
