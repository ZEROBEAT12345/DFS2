...
float4 FragmentFunction(v2f_t input) : SV_Target0
{
	...
	float3 vertex_bitan = normalize(-cross(input.normal, input.tangent.xyz));

	// output normal as color
	float4 finalColor = float4((vertex_bitan + float3(1.0,1.0,1.0)) / 2, 1.0);
