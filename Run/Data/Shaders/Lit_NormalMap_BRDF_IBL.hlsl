...
// IBL radiance map
TextureCube<float4> tIrradianceMap : register(t5);
TextureCube<float4> tPreFilterMap : register(t6); 
Texture2D<float4> tbrdfLUT : register(t7); 
...
float4 FragmentFunction(v2f_t input) : SV_Target0
{
	...
	// Calculate IBL texture color
	float4 irradiance_color = tIrradianceMap.Sample(sAlbedo, world_normal);
	const float MAX_REFLECTION_LOD = 4.0;
	float3 N = normalize(world_normal);
	float3 V = normalize(CAMPOS.xyz - input.worldPos);
	float3 R = reflect(-V, N);
	float4 prefiltered_color = tPreFilterMap.SampleLevel(sAlbedo, R, roughness_color.x * MAX_REFLECTION_LOD);
	float4 envBRDF_color = tbrdfLUT.Sample(sAlbedo, float2(saturate(dot(N, V)), roughness_color.x));

	float3 light = Get_IBL_BRDF_Lighting(CAMPOS.xyz,
		input.worldPos,
		world_normal,
		diffuse_Color.xyz,
		metallic_color.x,
		roughness_color.x,
		irradiance_color.xyz,
		prefiltered_color.xyz,
		envBRDF_color.xy);
